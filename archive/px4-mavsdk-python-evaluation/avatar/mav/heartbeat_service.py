...
class HeartbeatService:
    ...
    def record_heartbeat(self, name: str, timestamp: Optional[float] = None) -> None:
        ...
        Args:
            ...
            timestamp: Optional wall-clock time (seconds). Defaults to now.
        ...
        if name not in self._sources:
            ...
            self._sources[name].last_beat = ts

        logger.debug(f"Heartbeat recorded from '{name}' at {ts}")

    def get_last_heartbeat(self, source: str) -> Optional[float]:
        """Return wall-clock timestamp of last heartbeat for *source*, or None."""
        if source not in self._sources:
            return None
        lb = self._sources[source].last_beat
        return lb if lb > 0.0 else None
    ...
    async def stop(self) -> None:
        """Stop the monitor task and wait for clean shutdown (tests + server)."""
    ...
    async def __aexit__(
        ...
        """Async context manager exit — await async stop."""
        await self.stop_async()
