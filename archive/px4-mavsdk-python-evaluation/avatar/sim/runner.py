...
class SimTier(str, Enum):
    ...
    OFFLINE = "offline"  # No simulator required
...
class MockMcpClient:
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "flight_recorder_load":
            self._state["replay_loaded"] = True
            return {"status": "success", "success": True}
        if name == "flight_recorder_replay":
            self._state["replay_ok"] = True
            self._state["max_state_delta"] = 0
            return {"status": "success", "success": True, "max_state_delta": 0}
