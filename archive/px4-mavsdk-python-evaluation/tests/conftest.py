...
def pytest_runtest_setup(item: pytest.Item) -> None:
    """Reset process-wide singletons so test order does not leak across modules.

    ``avatar.mcp_server.tools.primitives`` and ``flight_tools`` each keep their
    own ``_state_machine`` global; tests often set only one. ``OffboardOwner`` is
    a process singleton whose asyncio lock must match the active loop. Clearing
    these before every test keeps the full suite deterministic.

    ``ConnectionManager`` is also a singleton; an earlier test may leave a
    cached MAVSDK ``System`` on it while later tests patch ``ConnectionManager``
    only at import time. Dropping the singleton forces a fresh manager per test.
    """
    import avatar.mav.offboard_owner as offboard_owner
    from avatar.mav.connection_manager import ConnectionManager
    from avatar.mcp_server.tools import flight_tools
    from avatar.mcp_server.tools import primitives

    offboard_owner._singleton = None  # type: ignore[attr-defined]
    ConnectionManager._instance = None  # type: ignore[misc]
    flight_tools.set_state_machine(None)
    primitives.set_state_machine(None)
